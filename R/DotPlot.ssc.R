#' DotPlot.ssc with sample mean as each dot, weighted mean as bar.
#'
...
DotPlot.ssc <- function (input, gene, group.by, split.by = NULL, point.by = "Patient",
                         num.cutoff = 10, num.cutoff.rm = F, title = "", number_labels = T,
                         text_sizes = c(20, 10, 10, 10, 5, 5, 3), spread = NULL, theme = "classic",
                         jitter.alpha = 0.5, jitter.width = 0.2, jitter.size = 2, jitter.col = T,
                         col.trans = "", bar = T, link = FALSE)
{
  # keep the order
  ...
  tmp <- tapply(input@assays$RNA@data[gene, ],
                apply(meta[, c(group.by, split.by), drop=F], 1,
  ...
  if (number_labels == T) {
    ...
    g <- g + geom_text(data = df.stat, aes_string(x = group.by, y = -max(df$gene)/10,
                                                  label = "frac_sum"), size = text_sizes[7])
  }
  if (theme == "bw") {
    g <- g + theme_bw()
  }
  ...
  if (jitter.col){
    ...
    g <- g + geom_jitter(aes_string(x = group.by, y = "gene"), width = jitter.width,
